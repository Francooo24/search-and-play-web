...
export async function POST() {
  ...
  try {
    ...
    // Per-game scores for game-specific achievements
    const { rows: gameScores } = await pool.query(
      "SELECT game, MAX(score) as best FROM leaderboard WHERE user_id = $1 GROUP BY game",
      [userId]
    );
    const gameMap: Record<string, number> = {};
    for (const g of gameScores) gameMap[g.game] = Number(g.best);
      ...
      if (a.game_specific) {
        ...
        if (a.condition_type === "games_played") {
          const { rows } = await pool.query(
            "SELECT COUNT(*) as c FROM leaderboard WHERE user_id = $1 AND game = $2",

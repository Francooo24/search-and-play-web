  ...
  return (
    <button
      ...
      className={`absolute top-3 right-3 z-10 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed
        ${saved ? "bg-amber-400/20 border border-amber-400/40 shadow-[0_0_10px_rgba(251,191,36,0.35)]" : "bg-white/5 border border-white/10 hover:border-amber-400/30 hover:bg-amber-400/10"}`}
    >
      <svg className={`w-4 h-4 transition-all duration-200 ${saved ? "text-amber-400 drop-shadow-[0_0_6px_rgba(251,191,36,0.8)]" : "text-gray-500 hover:text-amber-400"}`} viewBox="0 0 24 24" fill={saved ? "currentColor" : "none"} stroke="currentColor" strokeWidth={saved ? 0 : 1.8}>
